...
from .disciplina import Disciplina  # Importando o modelo Disciplina
...
class HorariosDisciplinas(models.Model):
    ...
    disciplina = models.ForeignKey(Disciplina, on_delete=models.CASCADE, related_name='horarios_disciplinas', null=True)
    ...
    def __str__(self):
        disciplina_info = self.disciplina if self.disciplina else "Sem disciplina"
        return f"{disciplina_info} ({self.horario_curso.hora_inicio} - {self.horario_curso.hora_fim}) - {self.horario_curso.curso} - {self.ano_semestre} - {self.periodo} - {self.dia_semana.nome} - {self.curso.nome}"
