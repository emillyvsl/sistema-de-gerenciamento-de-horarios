...
def horarioDisciplina(request):
    ...
    semestres = Semestre.objects.all().order_by('id')  # Ordenação dos semestres
    ano = request.GET.get("ano")
    semestre_id = request.GET.get("semestre")
    dias_semana = DiasSemana.objects.all()
    pesquisa_realizada = False

    # Ordena horários pelo período e horário de início para garantir a ordem crescente
    horarios = (
        ...
        .order_by("periodo", "horario_curso__hora_inicio")  # Ordena por período e horário
    )

    if ano and semestre_id:
        horarios = horarios.filter(
            ano_semestre__ano=ano, ano_semestre__semestre_id=semestre_id
        )
        ...
        try:
            ...
            messages.warning(
                request,
                "Para pesquisar, selecione o ano e o semestre. Exibindo o quadro mais recente.",
            )
    ...
    if horarios:
        for horario in horarios:
            ...
            if chave_horario not in horarios_vistos:
                ...
                horario.alocacoes_list = (
                    alocacoes if alocacoes.exists() else [{"disciplina": None}]
                )
...
def remover_alocacao(request, alocacao_id):
    ...
    if request.method == "POST":
        alocacao.disciplina = None
        alocacao.save()

        messages.success(request, "Disciplina removida da alocação com sucesso.")
